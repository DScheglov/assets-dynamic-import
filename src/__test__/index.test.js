...
describe('appendScript', () => {
  ...
  it('appends script to the body', async () => {
    ...
    expect(document.body.lastChild.tagName).toMatch(/^script$/i);
    expect(document.body.lastChild.src).toMatch(/^https?:\/\/domain\/script.js$/);
  });

  it('appends script to the body and allows to specify some atrributes', async () => {
    await appendScript('//domain/script.js', { id: 'script-1' });
    expect(
      document.getElementById('script-1')
    ).toBe(
  ...
  it('appends script to the body and allows to specify resolver', async () => {
    ...
    expect(
      await appendScript('//domain/script.js', () => moduleSymbol)
    ).toBe(moduleSymbol);
  });
  ...
  it('appends script to the body and allows to specify attributes and resolver', async () => {
    const moduleSymbol = Symbol('module');
    expect(
      await appendScript('//domain/script.js', { id: 'script-1' }, () => moduleSymbol)
    ).toBe(moduleSymbol);
...
describe('appendStyle', () => {
  ...
  beforeEach(() => {
    document.head.childNodes.forEach(child => document.head.removeChild(child));
    nativeConsoleError = console.error;
    console.error = () => {}; // suppressing output
  });
  ...
  it('appends style to the body', async () => {
    ...
    expect(document.head.lastChild.tagName).toMatch(/^link$/i);
    expect(document.head.lastChild.href).toMatch(/^https?:\/\/domain\/styles.css$/);
  });
  ...
  it('appends style to the body and allows to specify resolver', async () => {
    const moduleSymbol = Symbol('module');
    expect(
      await appendStyle('//domain/styles.css', () => moduleSymbol)
    ).toBe(moduleSymbol);
  });
  ...
  it('appends style to the body and allows to specify attributes and resolver', async () => {
    const moduleSymbol = Symbol('module');
    expect(
      await appendStyle('//domain/styles.css', { id: 'style-1' }, () => moduleSymbol)
    ).toBe(moduleSymbol);
...
describe('importScript', () => {
  ...
  it('caches failed requests', () => {
    ...
    expect(
      importScript('//domain/script.js?deny=true')
    ).toBe(res);
  ...
  it('allows to override cache with .force method', () => {
    ...
    expect(
      importScript('//domain/script-1.js')
    ).toBe(res2);
...
describe('importStyle', () => {
  ...
  it('caches failed requests', () => {
    ...
    expect(
      importStyle('//domain/styles.css?deny=true')
    ).toBe(res);
  ...
  it('allows to override cache with .force method', () => {
    ...
    expect(
      importStyle('//domain/styles-1.css')
    ).toBe(res2);
